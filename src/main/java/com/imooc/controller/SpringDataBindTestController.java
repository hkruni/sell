...
@Controller
public class SpringDataBindTestController {
...
//    </admin>
    ...
    @RequestMapping(value = "Converter.do",method = RequestMethod.POST)
    @ResponseBody
     public String date2(@Valid User user,Errors error){

        if(error.hasErrors()) {
            List<FieldError> errorLsit = error.getFieldErrors();
            errorLsit.forEach(e -> System.out.println("field :" + e.getField() +"\t" +
    ...
    @RequestMapping(value = "Converter1.do",method = RequestMethod.POST)
    @ResponseBody
    public String date2(Date mydate){
    ...
    @RequestMapping(value = "converter1.do")
    @ResponseBody
